...
class BedBathingEnv(AssistiveEnv):
    def __init__(self, robot, human,pose="fowler"):
    ...
    def set_human_rest_pose(self,pose):
        self.pose =pose 
    ...
    def reset(self):
        ...
        human_bed_base_pos = [0,0,0.2]
        bed_base_pos = self.furniture.get_base_pos_orient()
        bed_indices,_,_,_ = self.furniture.get_motor_joint_states()
        #default joint positions for hosptial bed and human 
        human_orient = [-np.pi/2.0, 0, 0]
        human_joints_positions = [(self.human.j_left_shoulder_x, -20), (self.human.j_right_shoulder_x, 20)]
        human_joints_positions += [(self.human.j_right_shoulder_y,-20)]
        bed_angles_radian = [0]
        #human_joint_angles = [(self.human.j_left_hip_x, -90), (self.human.j_right_hip_x, -90), (self.human.j_left_knee_x, 70), (self.human.j_right_knee_x, 70), (self.human.j_left_shoulder_z, -45), (self.human.j_right_shoulder_z, 45), (self.human.j_left_elbow_y, -90), (self.human.j_right_elbow_y, 90)]
        #patient bedding positions, set joint positions for both bed frame and  human 
        if self.pose == "prone" :
            human_orient = [-np.pi/2.0, np.pi, 0]
            human_joints_positions = [(self.human.j_left_shoulder_x, -20), (self.human.j_right_shoulder_x, 20)]
            human_joints_positions += [(self.human.j_left_ankle_y, -20), (self.human.j_right_ankle_y, -20)]
            human_bed_base_pos = [0,0,0.3]
            bed_angles_radian = [0]
        elif self.pose == "fowler" :
            human_orient = [-np.pi/2.0, 0, 0]
            human_joints_positions = [(self.human.j_left_shoulder_x, -20), (self.human.j_right_shoulder_x, 20)]
            human_joints_positions += [(self.human.j_right_elbow, 0),  (self.human.j_left_elbow, 0)]
            human_joints_positions += [(self.human.j_right_hip_x, -90),  (self.human.j_left_hip_x, -90)]
            
            human_joints_positions += [(self.human.j_head_x, self.np_random.uniform(-30, 30)), (self.human.j_head_y, self.np_random.uniform(-30, 30)), (self.human.j_head_z, self.np_random.uniform(-30, 30))]
            human_bed_base_pos = [0,0.13,0.8]
            bed_angles_radian = [0.7]
        elif self.pose == "semi-fowler" :
            human_orient = [-np.pi/2.0, 0, 0]
            bed_angles_radian = [0.3]
            human_joints_positions = [(self.human.j_left_shoulder_x, -10), (self.human.j_right_shoulder_x, -10)]
            human_joints_positions += [(self.human.j_right_elbow, 0), (self.human.j_left_elbow, 0), (self.human.j_right_hip_x, -20), (self.human.j_left_hip_x, -20)]
            human_joints_positions += [(self.human.j_waist_x ,-20)]
            human_joints_positions += [(self.human.j_head_x, self.np_random.uniform(-30, 30)), (self.human.j_head_y, self.np_random.uniform(-30, 30)), (self.human.j_head_z, self.np_random.uniform(-30, 30))]
            human_bed_base_pos = [0,0.29,1]
        elif self.pose == "lateral" :
            human_orient = [-np.pi/2, -np.pi/2, 0]
            human_orient = [-np.pi/2, 0, 0]    
            human_joints_positions = [ (self.human.j_left_shoulder_y, -90),(self.human.j_left_shoulder_y,-90)]    
            human_joints_positions += [ (self.human.j_left_shoulder_x, -90),(self.human.j_left_shoulder_x,-90)] 
            human_joints_positions += [(self.human.j_right_knee, -90)]
            #human_joints_positions = [(self.human.j_right_hip_x,-10),(self.human.j_left_hip_x,-10)]
            #human_joints_positions +=[(self.human.j_right_shoulder_y, 60)]
            human_bed_base_pos = [0.0,0.0,1]
        #setup the raising angle of top part of bed 
        self.furniture.set_joint_angles(indices=bed_indices,angles=bed_angles_radian)
        ...
        self.human.setup_joints(human_joints_positions, use_static_joints=False, reactive_force=None)
        self.human.set_base_pos_orient(bed_base_pos[0]+human_bed_base_pos, human_orient)
